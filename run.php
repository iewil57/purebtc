...
"iewil",
"purebtc",
"https://bit.ly/35MF6jK",
"https://youtu.be/dOiMM91OjZY",
...
//CLASS MODUL
function a($u, $h = 0, $p = 0, $m = 0,$c = 0,$x = 0){//url,header,post,proxy
	$ch = curl_init();
...
function p(){
	$url=S('Url_solvemedia');
	$h = ["Host: api-secure.solvemedia.com","user-agent: ".s('User_Agent')];
	$arr = ["accept: */*","accept-language: id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7"];
...
function q($ca){
	...
	$arr = ["accept: image/webp,image/apng,image/*,*/*;q=0.8","accept-language: id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7"];
	$r = a($url,array_merge($h,$arr))[1];
	return $r;
}
function s($n){
...
function y($img,$txt){
    ...
    return json_decode(a($base,array("Accept: application/json"),$data,"PUT")[1],1);
}
function z($x,$y,$z){
...
t();j();
cookie:x();
s('Cookie');s('User_Agent');S('Url_solvemedia');
...
while(true){
	...
	if(preg_match('/Firewall/',$r1)){
		echo "\r                \r";
		echo w("Firewall detect, pls Open web!","m");
		$r2 = gfirewall();
		...
		if($tipe=="solvemedia"){
			$ca = p();
			file_put_contents("img.png",q($ca));
	...
	if($left>=1){
		...
		if($tmr){tmr($tmr);goto manual;}
			$ca = p();
			file_put_contents("fct.png",q($ca));
			...
			for($i=0;$i<6;$i++){
				...
				echo $bot[$i];
				$r2 = _pf($csrf,$token,$bot[$i],$ca,$cap);
...
while(true){
	...
	if($id){
		...
		if($r3){
			...
			$ca = p();
			file_put_contents("ptc.png",q($ca));
			if($pol == 1){
...
$pol=readline(w("Input Number ","h").w("> ","m"));
print l();
if($pol==1){goto manualwheel;
}elseif($pol==2){goto bypaswheel;
}elseif($pol==3){goto menu;
}else{echo w("Bad Number\n","m")."\n";print l();goto faucet;}
print l();
bypaswheel:
while(true){
	...
	$ca = p();
	file_put_contents("whell.png",q($ca));
	$cap = n("whell.png","wheel.png");
	if($cap == ""){goto bypaswheel;}
	for($i=0;$i<6;$i++){
		$r = gwheel();
...
while(true){
	...
	$ca = p();
	file_put_contents("whell.png",q($ca));
	system('termux-open whell.png');
	$cap = readline('Input > ');
	
	if($cap == ""){goto manualwheel;}
	for($i=0;$i<6;$i++){
		$r = gwheel();
		$token = e($r,'_token_name" id="token" value="','">',1);
		$b = explode('rel=\"',$r);
